...
let year: HTMLElement | null               // "year" can be HTMLElement OR null
year = document.getElementById('year')     // get element with id "year"
let thisYear: string = new Date().getFullYear().toString() // current year as string
if (year) {
...
const year1 = document.getElementById('year') as HTMLSpanElement // force type
...
class coder {
    ...
    language: string // must be assigned in constructor, else error

    // constructor defines and initializes properties
    constructor(
        language: string,
        public readonly name: string, // public & readonly (can’t be changed later)
        public music: string,         // public property
        private number: number,       // private (only accessible inside class)
...
interface musician {
    name: string;                          // property: name of musician
    instrument: string;                    // property: instrument they play
    play(action: string): string;          // method: must return a string
}

class guitarist implements musician {
    name: string;         // required by interface
    instrument: string;   // required by interface

    // constructor initializes properties
    constructor(name: string, inst: string) {
        this.name = name
