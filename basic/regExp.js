...
let res = /(\d+)([a-z]+)/.exec("123abc456");
console.log(res) // ['123abc', '123' , 'abc']
/*
...
console.log("hello 123".replace(/\d+/, "#"));   // "hello #"
...
console.log("1,2,3".replaceAll(/\d/g, "X")); // "X,X,X"
/*
...
console.log(/abc/i.test("ABC")); // true (ignore case)
...
console.log("abc123".match(/\d+(?=3)/));   // ["12"]  (before 3)
console.log("abc123".match(/(?<=c)\d+/));  // ["123"] (after 'c')
/*
Regex: /\d+(?=3)/
...
// 2. Word Boundaries
console.log("cat scatter".match(/\bcat\b/)); // ["cat"] (full word only)
/*
Regex: /\bcat\b/
...
// 3. Quantifiers
console.log("aaa".match(/a{2,3}/)); // ["aaa"] (2–3 times)
console.log("aaab".match(/a+?/));   // ["a"]  (lazy match)
/*
Regex: /a{2,3}/
